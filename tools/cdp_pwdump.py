# -*- coding: utf-8 -*-
"""Dump page text and RSC response body to files with UTF-8."""
import json
import time
...
def main():
    ...
    for t in targets:
        if t.get('type') == 'page' and 'signin.ollama.com' in t.get('url', ''):
            ...
            txt = ev('document.body ? document.body.innerText : ""') or ''
            with open(r'D:\PRO\ollama-register\captures\pw-page-text.txt', 'w', encoding='utf-8') as f:
                f.write(txt)
            print('text saved:', len(txt))

            html = ev('document.documentElement.outerHTML') or ''
            with open(r'D:\PRO\ollama-register\captures\pw-page-html.txt', 'w', encoding='utf-8') as f:
                f.write(html)
