# -*- coding: utf-8 -*-
"""测试: Playwright 加载真实扩展后 Turnstile 是否正常(不再 600010)。"""
import json
import logging
import sys
import time
...
EXT_PATH = r'D:\PRO\ollama-register\ext\minimal'


def main():
    with sync_playwright() as p:
        browser = p.chromium.launch_persistent_context(
            user_data_dir=r'E:\down\Temp\pw-ext-test',
        ...
        # check chrome.runtime real
        fps = page.evaluate("""() => ({
            webdriver: navigator.webdriver,
            chromeRuntime: typeof (window.chrome && window.chrome.runtime),
            chromeRuntimeId: (window.chrome && window.chrome.runtime && window.chrome.runtime.id) || 'none',
        ...
        }""", 'ext.test.' + str(int(time.time())) + '@duck.com')
        ...
        for i in range(30):
            ...
            if state['err'] and 'human' in state['err']:
                log.info('>>> 失败: %s', state['err'])
                break

        page.screenshot(path=r'D:\PRO\ollama-register\captures\ext-test.png')
        browser.close()
