# -*- coding: utf-8 -*-
"""带完整 stealth + CDP 抓包的注册流程 v2。
修复: 使用 src/stealth.py 的完整伪造(chrome.runtime 等), Turnstile 渲染正常。
"""
import json
import logging
import sys
    ...
    def worker():
        for attempt in range(40):
            try:
                ...
                log.info('[capture] CDP connected')
                mid = 0
                ...
                def wait(mid_, timeout=15):
                    ...
                    while time.time() < end:
                        ...
                        if msg.get('id') == mid_:
                            return msg
                        if m.startswith('Network.') or m.startswith('Runtime.'):
                            capture_events.append(msg)
                            if m == 'Network.requestWillBeSent':
                                req = msg['params']['request']
                                url = req.get('url', '')
                                if ('ollama.com' in url or 'cloudflare.com' in url or 'workos.com' in url) and 'data:' not in url:
                                    log.info('[CAP] >>> %s %s', req.get('method'), url[:130])
                                    if req.get('postData'):
                                        log.info('[CAP]     BODY: %s', req['postData'][:300])
                                ...
                                try:
                                    ...
                                    txt = ' '.join(str(a) for a in args)[:200]
                                    if 'turnstile' in txt.lower() or 'error' in txt.lower():
                                        log.info('[console] %s', txt)
                                except Exception:
                                    pass
                    return None
...
def wait_turnstile_and_submit(page, timeout=300):
    log.info('>>> 请在浏览器窗口点击 Turnstile 验证框 (interaction-only, 需要真人点击)')
    start = time.time()
...
def main():
    ...
    ok = run(email, password, debug_port=9340)
    log.info('结果: %s', '成功' if ok else '失败')
    with open(r'D:\PRO\ollama-register\captures\cap-v2.json', 'w', encoding='utf-8') as f:
