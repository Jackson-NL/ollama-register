...
const tempBase = mailbox.base || 'https://mail.chatgpt.org.uk';
const tempJwt = mailbox.jwt || mailbox.apiKey || process.env.GPTMAIL_API_KEY;
// 邮箱 Key 池：GPTMAIL_API_KEY=key1,key2 或 GPTMAIL_API_KEY 单 Key，429 立即轮换
const rawChatKeys = (process.env.GPTMAIL_API_KEY || process.env.GPTMAIL_API_KEY || '').split(',').map(s=>s.trim()).filter(Boolean);
...
function persistDeleteChatKey(k){
  try{
    ...
    const m = env.match(/GPTMAIL_API_KEY=.*/);
    if(m){
      const keys = m[0].replace('GPTMAIL_API_KEY=','').split(',').map(s=>s.trim()).filter(Boolean);
      const filtered = keys.filter(x=>x!==k);
      if(filtered.length !== keys.length){
        env = env.replace(/GPTMAIL_API_KEY=.*/, `GPTMAIL_API_KEY=${filtered.join(',')}`);
...
async function listMails() {
  const isGptMail = /chatgpt\.org\.uk/i.test(tempBase) || chatEmailId;
  if (isGptMail) {
    async function gptFetch(url){
      const key = mailbox.apiKey || process.env.GPTMAIL_API_KEY || '';
      const res = await fetch(url, { headers: { 'X-API-Key': key } });
      if(res.status===429){
        const txt = await res.clone().text().catch(()=> '');
        let j={}; try{ j=JSON.parse(txt);}catch{}
        log(`GPTMAIL_429 ${url} remaining:${j.remaining||''}`);
        await sleep(1500);
        return res;
      }
      if(res.status===401){
        log(`GPTMAIL_401 ${url} key ${key.slice(0,8)}...`);
        throw new Error(`GPTMAIL_401 ${key.slice(0,8)}...`);
      }
      return res;
    }
    const res = await gptFetch(tempBase + '/api/emails?email=' + encodeURIComponent(email));
    const txt = await res.text();
    if(!res.ok) throw new Error('gptmail list ' + res.status + ': ' + txt.slice(0,500));
    const data = JSON.parse(txt);
    const msgs = data.data?.emails || data.emails || [];
    const detailed = [];
    for(const m of msgs.slice(0,10)){
      try{
        const dr = await gptFetch(tempBase + '/api/email/' + m.id);
        const dt = await dr.text();
        if(dr.ok){
          const dm = JSON.parse(dt);
          const d = dm.data || dm;
          detailed.push({ id: d.id||m.id, to: d.email_address||email, subject: d.subject||m.subject||'', text: d.content||d.text||'', html: d.html_content||d.html||'' });
        } else {
          detailed.push({ subject: m.subject||'', text: m.content||'', html: m.html_content||'' });
        }
      }catch{}
    }
