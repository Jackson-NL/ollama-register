...
(async () => {
  ...
  try {
    ...
    const isLogin = (process.env.OLLAMA_MODE||'').toLowerCase()==='login' || process.env.OLLAMA_USE_LOGIN==='1';
    const signupUrl = isLogin
      ? `https://signin.ollama.com/sign-in?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirect)}`
      : `https://signin.ollama.com/sign-up?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirect)}${sid ? `&authorization_session_id=${encodeURIComponent(sid)}` : ''}`;
    log('mode', isLogin?'login':'signup', 'url', signupUrl);
