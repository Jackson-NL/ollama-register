...
const mailboxPath = process.env.OLLAMA_MAILBOX || path.resolve(root, 'exports/chattempmail_test.json');
const mailbox = JSON.parse(fs.readFileSync(mailboxPath, 'utf8'));
const email = process.env.OLLAMA_SIGNUP_EMAIL || mailbox.address;
const tempBase = mailbox.base || 'https://chat-tempmail.com';
const tempJwt = mailbox.jwt || mailbox.apiKey || process.env.CHAT_TEMPMAIL_API_KEY;
// 邮箱 Key 池：CHAT_TEMPMAIL_API_KEYS=key1,key2 或 CHAT_TEMPMAIL_API_KEY 单 Key，429 立即轮换
const rawChatKeys = (process.env.CHAT_TEMPMAIL_API_KEYS || process.env.CHAT_TEMPMAIL_API_KEY || '').split(',').map(s=>s.trim()).filter(Boolean);
const chatKeyPool = rawChatKeys.length ? rawChatKeys : (mailbox.apiKey ? [mailbox.apiKey] : []);
let chatKeyIdx = 0;
function persistDeleteChatKey(k){
  try{
    ...
    if(m){
      ...
      if(filtered.length !== keys.length){
        ...
        log(`[KEY_DELETED] ${k.slice(0,8)}... removed from .env (poll 429 ${keys.length}->${filtered.length})`);
        const delLog = path.join(root,'exports','chat_exhausted_deleted.jsonl');
        fs.appendFileSync(delLog, JSON.stringify({ ts: new Date().toISOString(), key: k.slice(0,8)+'...', reason: 'poll 429 remaining:0', runId })+'\n');
      }
    }
  }catch(e){ log('[KEY_DELETE_FAIL]', String(e.message||e).slice(0,200)); }
}
function currentChatKey(){ return chatKeyPool[chatKeyIdx % chatKeyPool.length] || ''; }
function rotateChatKey(reason){
  if(chatKeyPool.length<=1) return currentChatKey();
  chatKeyIdx = (chatKeyIdx+1) % chatKeyPool.length;
  log(`CHAT_KEY_ROTATE ${reason} -> idx ${chatKeyIdx} key ${currentChatKey().slice(0,8)}...`);
  return currentChatKey();
}
async function chatFetch(url, opts={}, retries=chatKeyPool.length){
  for(let attempt=0; attempt<retries; attempt++){
    const key = currentChatKey();
    const headers = { ...(opts.headers||{}), 'X-API-Key': key, 'Authorization': `Bearer ${key}` };
    const res = await fetch(url, { ...opts, headers });
    if(res.status===429){
      log(`CHAT_API_429 ${url} key ${key.slice(0,8)}... attempt ${attempt+1}/${retries}`);
      if(attempt < retries-1) { rotateChatKey('429'); await sleep(500); continue; }
    }
    return res;
  }
  // fallback
  const key = currentChatKey();
  return fetch(url, { ...opts, headers: { ...(opts.headers||{}), 'X-API-Key': key, 'Authorization': `Bearer ${key}` } });
}
const chatEmailId = mailbox.id || mailbox.emailId;
const chatApiKey = mailbox.apiKey || currentChatKey();
...
async function getOutlookAccessToken(){
  const cid = mailbox.client_id || process.env.OUTLOOK_CLIENT_ID || '9e5f94bc-e8a4-4e73-b8be-63364c29d753';
  const rt = mailbox.refresh_token || process.env.OUTLOOK_REFRESH_TOKEN || '';
  if(!rt) throw new Error('outlook refresh_token missing');
  const params = new URLSearchParams({ client_id: cid, refresh_token: rt, grant_type: 'refresh_token', scope: 'https://graph.microsoft.com/Mail.Read offline_access' });
  ...
  if(!res.ok) throw new Error('outlook token ' + res.status + ': ' + txt.slice(0,300));
  const j = JSON.parse(txt);
  return j.access_token;
}
async function listMails() {
  const isOutlook = /outlook/i.test(tempBase) || mailbox.refresh_token || /outlook\.com/i.test(email);
  if (isOutlook) {
    try{
      const token = await getOutlookAccessToken();
      const res = await fetch('https://graph.microsoft.com/v1.0/me/messages?$top=10&$select=subject,from,receivedDateTime,bodyPreview,body,internetMessageId', { headers:{ 'Authorization': `Bearer ${token}` }});
      const txt = await res.text();
      if(!res.ok) throw new Error('outlook list ' + res.status + ': ' + txt.slice(0,500));
      const data = JSON.parse(txt);
      const msgs = data.value || [];
      const detailed = msgs.map(m=>({
        id: m.id,
        to: email,
        subject: m.subject||'',
        text: (m.bodyPreview||'') + '\n' + ((m.body&&m.body.content)||''),
        html: (m.body&&m.body.content)||'',
        received: m.receivedDateTime
      }));
      return { results: detailed, mails: detailed };
    }catch(e){
      log('outlook poll error', String(e.message||e).slice(0,500));
      throw e;
    }
  }
  const isChatTemp = /chat-tempmail\.com/i.test(tempBase) || chatEmailId;
  if (isChatTemp) {
    if (!chatEmailId) throw new Error('chat-tempmail emailId missing in mailbox');
    // 邮箱归属固定为创建时的 Key，轮询必须用 mailbox 的 apiKey，不能用池轮换（否则 403 emailNotFoundOrNoPermission）
    async function chatFetchFixed(url){
      for(let attempt=0; attempt<3; attempt++){
        const res = await fetch(url, { headers: { 'X-API-Key': chatApiKey, 'Authorization': `Bearer ${chatApiKey}` } });
        if(res.status===401){
          const txt = await res.clone().text().catch(()=> '');
          log(`CHAT_POLL_401 ${url} key ${chatApiKey.slice(0,8)}... body:${txt.slice(0,120)} -> delete`);
          persistDeleteChatKey(chatApiKey);
          throw new Error(`CHAT_KEY_401 invalid key ${chatApiKey.slice(0,8)}...`);
        }
        if(res.status===429){
          const txt = await res.clone().text().catch(()=> '');
          let remaining=null;
          try{ const j=JSON.parse(txt); remaining=j.remaining; }catch{}
          if(remaining===0){
            log(`CHAT_POLL_EXHAUSTED ${url} key ${chatApiKey.slice(0,8)}... remaining:0 -> fail fast`);
            persistDeleteChatKey(chatApiKey);
            throw new Error(`CHAT_KEY_EXHAUSTED remaining:0 key ${chatApiKey.slice(0,8)}...`);
          }
          log(`CHAT_POLL_429 ${url} key ${chatApiKey.slice(0,8)}... wait ${1000*(attempt+1)}ms`);
          await sleep(1000*(attempt+1));
          continue;
        }
        return res;
      }
      return fetch(url, { headers: { 'X-API-Key': chatApiKey, 'Authorization': `Bearer ${chatApiKey}` } });
    }
    const res = await chatFetchFixed(`${tempBase}/api/emails/${chatEmailId}`);
    const text = await res.text();
    if (!res.ok) throw new Error(`chat-tempmail list ${res.status}: ${text.slice(0, 500)}`);
    const data = JSON.parse(text);
    const msgs = data.messages || [];
    const detailed = [];
    for (const m of msgs.slice(0, 10)) {
      try {
        const dr = await chatFetchFixed(`${tempBase}/api/emails/${chatEmailId}/${m.id}`);
        const dt = await dr.text();
        if (dr.ok) {
          const dmRaw = JSON.parse(dt);
          const dm = dmRaw.message || dmRaw;
          detailed.push({
            id: dm.id || m.id,
            to: dm.to_address || dm.to || '',
            subject: dm.subject || m.subject || '',
            text: dm.content || dm.text || '',
            html: dm.html || '',
          });
        } else {
          detailed.push({ subject: m.subject || '', text: m.from_address || '', html: '' });
        }
      } catch {}
    }
    return { results: detailed, mails: detailed };
...
async function pollCode(maxMs=maxMailWaitMs) {
  ...
  while (Date.now() - started < maxMs) {
    try {
      ...
      if(String(e.message||e).includes('CHAT_KEY_EXHAUSTED')){
        log('FATAL: mail key exhausted, aborting job');
        throw e;
      }
    }
    await sleep(wait);
