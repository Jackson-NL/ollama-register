...
async function listMails() {
  const isMailTm = /mail\.tm|emalupe\.com/i.test(tempBase) || /emalupe\.com|mail\.tm/i.test(mailbox.address || '');
  if (isMailTm) {
    const res = await fetch(`https://api.mail.tm/messages?page=1`, {
      headers: { Authorization: `Bearer ${tempJwt}` }
    });
    const text = await res.text();
    if (!res.ok) throw new Error(`mail.tm list ${res.status}: ${text.slice(0, 500)}`);
    const data = JSON.parse(text);
    const members = data['hydra:member'] || data.members || [];
    // fetch details for each message to get text/html
    const detailed = [];
    for (const m of members.slice(0, 10)) {
      try {
        const dr = await fetch(`https://api.mail.tm/messages/${m.id}`, {
          headers: { Authorization: `Bearer ${tempJwt}` }
        });
        ...
        if (dr.ok) {
          ...
          detailed.push({
            id: dm.id,
            to: dm.to ? JSON.stringify(dm.to) : '',
            subject: dm.subject || '',
            text: dm.text || dm.html || '',
            html: dm.html || '',
            intro: dm.intro || '',
          });
        } else {
          detailed.push({ subject: m.subject || '', text: m.intro || '', html: '' });
        }
      } catch {}
    }
